...
2. **Inverted behavioral dependency — this occurred under 0.55:** the original symlink-not-copy check rode on archgate's file reader _rejecting_ symlinks, which is not a semver-guaranteed contract, and the binary is a shared global cache (`~/.archgate/bin`) rather than a per-repo pin. archgate 0.55 began resolving symlinks inside the project root; the signature inverted exactly as predicted here, every valid symlink read as a forbidden copy, and all commits were blocked. The unit tests did not catch it because they mocked the non-following reader — that mock is now corrected to model 0.55. Resolved by re-basing the rule on **content equality**, which is documented and needs no frontmatter flag. Evidence: `docs/research/symlink-detection-055.md`; decision: issue #5.
